...
# Check if the required CRAN packages are installed
...
devtools::install_github(repo = "malucalle/selbal", force = TRUE)
...
#Read in ps object
(ps <- readRDS("E:/Downloads/ps_giloteaux_2016.rds"))
...
tree <- phyloseq::phy_tree(ps_rare)
...
adiv <- data.frame(
  ...
  "PD" = picante::pd(samp = data.frame(t(data.frame(phyloseq::otu_table(ps_rare)))),  tree = tree, include.root = TRUE),
...
adiv %>%
  gather(key = metric, value = value, c("Observed", "Shannon", "PD.PD")) %>%
  mutate(metric = factor(metric, levels = c("Observed", "Shannon", "PD.PD"))) %>%
...
wilcox.test(PD.PD ~ Status, data = adiv, conf.int = TRUE)
